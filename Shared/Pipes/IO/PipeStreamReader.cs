...
namespace NamedPipeWrapper.IO
{
		...
		where T : class
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="PipeStreamReader{T}"/> class that reads data from the given <paramref name="stream"/>.
		/// </summary>
		...
		private int ReadLength()
		{
			...
			int bytesRead = this.BaseStream.Read(lenbuf, 0, lensize);
			if (bytesRead == 0)
			{
		...
		private T ReadObject(int len)
		{
			...
			{
				return (T)this.binaryFormatter.Deserialize(memoryStream);
			}
		}
	}
}
