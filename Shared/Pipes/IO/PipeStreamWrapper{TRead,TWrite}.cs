...
namespace NamedPipeWrapper.IO
{
		...
		where TWrite : class
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="PipeStreamWrapper{TRead, TWrite}"/> class that reads from and writes to the given <paramref name="stream"/>.
		/// </summary>
		/// <param name="stream">Stream to read from and write to.</param>
		public PipeStreamWrapper(PipeStream stream)
		{
			this.BaseStream = stream;
			this.reader = new PipeStreamReader<TRead>(this.BaseStream);
			this.writer = new PipeStreamWriter<TWrite>(this.BaseStream);
		}
		...
		/// </returns>
		public bool IsConnected => this.BaseStream.IsConnected && this.reader.IsConnected;
		...
		/// </returns>
		public bool CanRead => this.BaseStream.CanRead;
		...
		/// </returns>
		public bool CanWrite => this.BaseStream.CanWrite;
